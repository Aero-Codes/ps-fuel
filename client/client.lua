...
RegisterNetEvent('ps-fuel:client:grabNozzleFromPump', function(data)
	local ped = PlayerPedId()
...
RegisterNetEvent('ps-fuel:client:buyCanMenu', function(data)
	...
	exports['qb-menu']:openMenu({
		...
		},
		{
			...
			icon = 'fas fa-gas-pump',
			params = {
				event = "ps-fuel:client:grabNozzleFromPump",
				args = {
					entity = data.entity
				}
			}
...
RegisterNetEvent('ps-fuel:client:PayFuel', function()
	local vehicle = QBCore.Functions.GetClosestVehicle()
	...
	if HasPedGotWeapon(ped, 883325847) then
		if GetAmmoInPedWeapon(ped, 883325847) ~= 0 then
			if CurFuel < 95 then
				TriggerServerEvent('ps-fuel:server:PayFuel', 0, inGasStation, true)
			else
...
AddEventHandler('ps-fuel:client:sendBill', function(data)
	...
	if success then
		if QBCore.Functions.GetPlayerData().money['cash'] <= total then
			...
			TriggerServerEvent('ps-fuel:server:PayFuel', data.totalcost, inGasStation, false)
